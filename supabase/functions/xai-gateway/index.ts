...
serve(async (req) => {
  ...
  try {
    // ── Auth: require valid JWT from an admin/super_admin user ──
    const authHeader = req.headers.get("Authorization");
    if (!authHeader?.startsWith("Bearer ")) {
      return errRes("Unauthorized — missing token", 401);
    }
    ...
    const { data: roleCheck } = await serviceClient.rpc("is_admin", { _user_id: userId });
    if (!roleCheck) {
      return errRes("Forbidden — admin access required", 403);
    }
