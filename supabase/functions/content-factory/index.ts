...
async function callXAI(messages: Array<{ role: string; content: string }>, model = "grok-3-mini-fast") {
...
// Stage 1: Generate viral video script via xAI
async function generateScript(product: any): Promise<string> {
...
async function generateVideo(product: any, script: string): Promise<string> {
...
${script.substring(0, 200)}`;
  ...
  const res = await fetch(`${XAI_BASE}/videos/generations`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${getXAIKey()}` },
    body: JSON.stringify({ model: "grok-imagine-video", prompt, duration: 10, aspect_ratio: "9:16", resolution: "720p" }),
  });

  const data = await res.json();
  if (!res.ok) throw new Error(`xAI video error [${res.status}]: ${JSON.stringify(data)}`);
  return data.request_id;
}

// Stage 3: Poll video status
async function pollVideoStatus(requestId: string) {
  const res = await fetch(`${XAI_BASE}/videos/${requestId}`, { headers: { Authorization: `Bearer ${getXAIKey()}` } });
  const data = await res.json();
  if (!res.ok) throw new Error(`xAI poll error: ${res.status}`);
  return data;
}

// Stage 4: Generate YouTube metadata via xAI
async function generateYouTubeMetadata(product: any, script: string) {
  const data = await callXAI([
    { role: "system", content: "You generate YouTube Shorts metadata. Return JSON only with: title (max 100 chars, emoji-rich), description (with hashtags and link placeholder), tags (array of 10-15 tags)." },
    { role: "user", content: `Generate YouTube Shorts metadata for:\nProduct: ${product.name || product.title}\nScript: ${script.substring(0, 300)}\n\nReturn JSON: { "title": "...", "description": "...", "tags": [...] }` },
  ]);
...
async function runPipeline(pipelineId: string, product: any) {
  ...
  const requestId = await generateVideo(product, script);
  ...
  const ytMetadata = await generateYouTubeMetadata(product, script);
  await updatePipeline(pipelineId, { metadata: { youtube: ytMetadata, script_generated_at: new Date().toISOString() } });
  ...
  try {
    ...
    if (action === "start_single") {
      const { data: entry, error } = await supabase.from("content_pipeline").insert({
        ...
        product_data: product, source: product.source || "manual",
        platform: "youtube_shorts", status: "processing", stage: "starting",
      }).select().single();
      if (error) throw new Error(`DB error: ${error.message}`);
      const result = await runPipeline(entry.id, product);
      return new Response(JSON.stringify({ success: true, pipeline_id: entry.id, ...result }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (action === "start_batch") {
      ...
      for (const prod of (products || []).slice(0, 5)) {
        const { data: entry, error } = await supabase.from("content_pipeline").insert({
          product_name: prod.name || prod.title, product_data: prod,
          source: prod.source || "viral_research", platform: "youtube_shorts",
    ...
    if (action === "poll_video") {
      const { data: entry } = await supabase.from("content_pipeline").select("*").eq("id", pipeline_id).single();
      if (!entry?.video_request_id) throw new Error("No video request found");
      const videoStatus = await pollVideoStatus(entry.video_request_id);
      if (videoStatus.status === "completed" && videoStatus.video) {
        await updatePipeline(pipeline_id, { video_url: videoStatus.video.url || videoStatus.video, stage: "video_ready", status: "ready_to_post" });
      }
      return new Response(JSON.stringify({ success: true, ...videoStatus }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    if (action === "get_pipeline") {
      const { data } = await supabase.from("content_pipeline").select("*").order("created_at", { ascending: false }).limit(50);
